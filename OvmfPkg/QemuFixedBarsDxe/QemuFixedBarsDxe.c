 ...
 */
#define PPB_MEM32_BASE_OFFSET       0x20   /* non-prefetchable, 32-bit */
#define PPB_MEM32_LIMIT_OFFSET      0x22
#define PPB_PREF_MEM_BASE_OFFSET    0x24   /* prefetchable, 64-bit capable */
...
/*
 * Read the prefetchable BAR ranges from an endpoint by looking up its
 * programmed BAR addresses in the blob.  Since fixed-bar=on guarantees
 * every endpoint under the RP has fixed BARs, a match is always found.
 */
  ...
  )
{
  ...
  for (Off = PCI_BASE_ADDRESSREG_OFFSET; Off <= 0x24; ) {
    ...
    if ((Lo & 0x6) == 0x4) {        /* 64-bit memory BAR */
      ...
      /* Match address against blob to get size and type. */
      for (DevIdx = 0; DevIdx < mNumDevices; DevIdx++) {
 ...
 * prefetchable ranges from all endpoints below it (bottom-up), program
 * the bridge window, and return the union to the caller.
 *
 * Because fixed-bar=on requires every endpoint with memory BARs under the
 * RP to have pci-bars= specified, EndpointRanges() only returns non-empty
